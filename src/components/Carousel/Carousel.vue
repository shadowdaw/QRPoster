...
<template>
	...
	<div class="v-carousel">
		...
		<div class="v-carousel-nav prev" @click="prev" v-if="control" v-show="hasLoop || (itemsLen > 1 && activeIndex > 0)"
			 v-html="prevHtml"></div>
		<div class="v-carousel-nav next" @click="next" v-if="control" v-show="hasLoop || (itemsLen > 1 && activeIndex < itemsLen - 1)"
			 v-html="nextHtml"></div>
...
export default {
	props: {
		...
		*/
		...
		// control-btn is available if it's set to true.
		control: {
			type: Boolean,
			default: true,
		},
	...
	*/
	...
	methods: {
		...
		to(index,nocalc) {
			...
			if(!nocalc){me.updateHeight(realIndex);} 
		},
		checkTrans() {
			const me = this,
		...
		updateHeight(idx) { 
			if(this.calcHeight && this.items.length){ 
				doCSS(this.itemsWrap, 'height', `${this.items[idx].clientHeight}px`);
			} 
		},
		transTo(moveTo) {
